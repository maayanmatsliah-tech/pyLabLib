...
def copy_file(source, dest, overwrite=True, cmp_on_overwrite=True, preserve_metadata=True):
    """
    ...
    Args:
        ...
        preserve_metadata (bool): If ``True``, preserve file metadata (such as creation time) by using :func:`shutil.copy2`; otherwise, use :func:`shutil.copy`
    """
...
def copy_dir(source, dest, folder_filter=None, file_filter=None, overwrite=True, cmp_on_overwrite=True, preserve_metadata=True):
    """
    ...
    for path,_,files in walk_dir(source,folder_filter=folder_filter,file_filter=file_filter):
        ...
        for f in files:
            ...
            copy_file(source_path,dest_path,overwrite=overwrite,cmp_on_overwrite=cmp_on_overwrite,preserve_metadata=preserve_metadata)
...
def retry_copy(source, dest, overwrite=True, cmp_on_overwrite=True, preserve_metadata=True, try_times=5, delay=0.3):
    """
    ...
    """
    general.retry_wait(lambda: copy_file(source,dest,overwrite,cmp_on_overwrite,preserve_metadata), try_times, delay)
...
def retry_copy_dir(source, dest, folder_filter=None, file_filter=None, overwrite=True, cmp_on_overwrite=True, preserve_metadata=True, try_times=5, delay=0.3):
    """
    Retrying version of :func:`copy_dir` (see :func:`retry_copy` for details on retrying).
    """
    general.retry_wait(lambda: copy_dir(source,dest,folder_filter,file_filter,overwrite,cmp_on_overwrite,preserve_metadata), try_times, delay)
...
def zip_folder(zip_path, source_path, inside_path="", folder_filter=None, file_filter=None, mode="a", compression=zipfile.ZIP_DEFLATED, compresslevel=None):
    ...
    Args:
        ...
        compresslevel: Zip archive compression level (see :class:`zipfile.ZipFile`); ignored for Python version below 3.7.
    """
    with _ZipFile(zip_path, mode=mode, compression=compression, compresslevel=compresslevel) as zf:
...
def zip_file(zip_path, source_path, inside_name=None, mode="a", compression=zipfile.ZIP_DEFLATED, compresslevel=None): 
    """
...
def zip_multiple_files(zip_path, source_paths, inside_names=None, mode="a", compression=zipfile.ZIP_DEFLATED, compresslevel=None): 
    """
    ...
    for sp,inm in zip(source_paths,inside_names):
        zip_file(zip_path,sp,inside_name=inm,mode=mode,compression=compression,compresslevel=compresslevel)
